...
public class SimulatedStepperMotorTest {
	...
	static com.pi4j.io.gpio.Pin[] pi4jPins = new com.pi4j.io.gpio.impl.PinImpl[4];
	static {
	for (int i = 0; i < pins.length; i++) {
		...
		pi4jPins[i] = new  com.pi4j.io.gpio.impl.PinImpl("", i, "GPIO " + i, EnumSet.of(PinMode.DIGITAL_OUTPUT));
	}
	}
	
	public HardwareBuilderFactory pcbf = new PluggedClassBuilderFactory();
	...
	private SimulatedStepperMotor getDeviceUnderTest(SimulatedStepperMotorProperties properties) {
		HardwareBuilder builder = pcbf.createBuilder(properties );
		SimulatedStepperMotor dut = new SimulatedStepperMotor(builder);
		dut.enterInitialize();
		return dut;
	}
	...
	public void testGetModelVariables() {
		SimulatedStepperMotor dut = getDeviceUnderTest(buildProperty());
		
		List<Fmi2ScalarVariable> variables = dut.getModelVariables();
		
		assertEquals(2,variables.size() );
		
		assertEquals("simulatedStepperMotor position",variables.get(0).getName() );
		assertNotNull(variables.get(0).getReal() );
		assertEquals("output",variables.get(0).getCausality() );
		assertEquals(0,variables.get(0).getValueReference());
		assertEquals("continuous",variables.get(0).getVariability());
		
		assertEquals("simulatedStepperMotor torque",variables.get(1).getName() );
		assertNotNull(variables.get(1).getReal() );
		assertEquals("input",variables.get(1).getCausality() );
		assertEquals(1,variables.get(1).getValueReference());
		assertEquals("continuous",variables.get(1).getVariability());			
	}
	...
	public void testInitals() throws Exception {
		...
		SimulatedStepperMotor dut = getDeviceUnderTest(buildProperty().setInitalPosition(0.5));
		...
		dut = getDeviceUnderTest(buildProperty().setInitalPosition(345));
	...
	public void testStep() throws Exception {
		SimulatedStepperMotor dut = getDeviceUnderTest(buildProperty().setStepsPerRotation(1024));
